// Strings used by options, help, and upgrade pages

il8n.strings = {

	Title: 'Tab Vault instellingen',				// Options page tab title
	By: 'Door',
	
	TitleHelp: 'Tab Vault hulp',			// Help page tab title
	TitleHelp2: 'Hulp',
	TitleUpgrade: 'Tab Vault upgrader',		// Upgrade page tab title
	TitleUpgrade2: 'Upgrader',
	
	Default: 'Standaard',						// Default button text
	ActionFailed: 'Mislukt!',				// Status message for errors
	ActionDone: 'Gelukt!',					// Status message for success
	
	Preferences: 'Voorkeuren',				// Preferences category header
	CompactTabs: '<Unused>',
	Tooltips: 'Laat tips zien in de popup',
	VerboseTabTips: 'Laat zowel de titel als de URL zien in de tabs hulplijn',
	TrashOnOpen: 'Verwijder tabs automatisch naar de prullenbak na het openen',
	DisableAnimation: 'Schakel gebruikersinterface animaties uit',
	SaveToTop: 'Voeg bewaarde tabs bovenaan de lijst toe',
	GroupToTop: 'Voeg gegroepeerde tabs bovenaan de lijst toe',
	CloseTabOnSave: 'Sluit de huidige tab na het bewaren ervan',
	CloseOnSave: 'Sluit de popup na het bewaren van de huidige tab',
	CloseOnPageOpen: 'Sluit de popup na het loslaten van een bewaarde tab boven de pagina',
	KeepGroupsOpen: 'Houd groepen uitgeklapt tijdens het heropenen van de popup',
	ShowBadge: 'Toon het aantal bewaarde tabs op een badge op de knop',
	BackgroundColor: 'Achtergrondkleur badge',
	BackgroundAlpha: 'Doorzichtigheid achtergrond badge',
	TextColor: 'Tekstkleur badge',
	TextAlpha: 'Tekstdoorzichtigheid badge',
	LimitHeight: 'Limiteer de hoogte van de popup',
	MaxHeight: 'Maximum hoogte van de popup (px)',
	PopupWidth: 'Popup width (px)',
	MiddleClick: 'Middelklik actie',
	MiddleClickOpen: 'Open tab in achtergrond',
	MiddleClickClose: 'Verwijder tab',
	MiddleClickNone: 'Doe niks',
	CxMenuStyle: 'Context menu stijl',
	Language: 'Taal (vereist herstart)',
	
	ImportExport: 'Import/Export',			// Import/Export category header
	ImportNote: '<strong>Let op:</strong> Het importeren van een sessie zal elke geïmporteerde tab openen om de favicons te verzamelen',
	ExportButton: 'Exporteer sessie',
	Export: 'Exporteer je bewaarde tabs als een sessie-bestand',
	ImportButton: 'Importeer sessie',
	Import: 'Importeer een sessie-bestand naar je bewaarde tabs',
	ImportWorking: 'Importeren...',
	ImportPlaceholder: 'Open een sessie-bestand (.win) in een textbewerkingsprogramma en kopiëer en plak de content hier voor het importeren.',
	
	
	Reset: 'Herstel',							// Reset category header
	ResetSettingsButton: 'Herstel instellingen',
	ResetSettings: 'Herstel alle instellingen naar hun standaardwaardes',
	ResetTabsButton: 'Leeg Kluis',
	ResetTabs: 'Verwijder alle bewaarde tabs van de kluis',
	ResetAllButton: 'Herstel alles',
	ResetAll: 'Herstel Tab Vault naar de oorspronkelijke status',
	
	ExternalAccess: 'Knoppen en shortcuts',
	EnableAccess: 'Stel een wachtwoord in om sneltoetsen toegang tot Tab Vault toe te staan',
	Password: 'Wachtwoord',
	NewPassword: 'Genereer nieuw wachtwoord',
	SaveTabAction: 'Bewaar tab actie - voor gebruik met sneltoetsen en muisgebaren',
	SaveTabButton: 'Bewaar tab actie - klik om op te slaan en sleep hem naar de gewenste werkbalk via het Uiterlijk-scherm',
	SaveButtonTitle: 'Bewaar met Tab Vault',
	
	
	Help: 'Hulp nodig?',						// Help category header
	HelpLink: 'Klik hier om de hulppagina te bekijken voor meer informatie over wat Tab Vault is, hoe het werkt en hoe je het kan gebruiken.',


	Upgrading: 'Je instellingen aan het updaten',	// Upgrade explanation header
	UpgradeMessage: 'Tab Vault 2 bewaart de instellingen iets anders dan versie 1, dus ' +
			'zal Tab Vault je instellingen nu updaten naar het nieuwe formaat om incompatibiliteit te voorkomen ' +
			'Tab Vault heeft je bewaarde tabs opgeslagen als sessiebestand. ' +
			'(Kijk naar je tabs. Ze zouden naast deze open moeten zijn.) Als de update mislukte, ' +
			'kan je deze sessie importeren om je tabs terug te krijgen.',

	UpgradeButton: 'Update nu!',
	UpgradeDone: 'Tab Vault is nu klaar om te gebruiken!',		// Upgrade success header
	UpgradeHelp: 'Zie wat nieuw is in versie 2',
	UpgradePrefs: 'Verander je voorkeuren',
	UpgradeClose: 'Ga weer browsen!',
	
	UpgradeStatus1: 'Huidige instellingen laden...',		// Upgrade status message
	UpgradeStatus2: 'Huidige tabs laden...',
	UpgradeStatus3: 'Legen van de extensie opslag...',
	UpgradeStatus4: 'Terugzetten instellingen...',
	UpgradeStatus5: 'Terugzetten tabs...',
	
	UpgradeFail1: 'Oeps! Er is iets fout gegaan (of je hebt een heel trage computer)',
	UpgradeFail2: 'Je kan helpen door op Ctrl+Shift+O te klikken om de foutenconsole te openen ' + 
			'en de fouten versturen na "Starting upgrade" in de Reported Issues sectie van ' +
			'<a href="https://addons.opera.com/addons/extensions/details/tab-vault/">Tab Vaults ' +
			'extensie pagina</a>. Bedankt!',

	Footer: '<a href="https://addons.opera.com/addons/extensions/details/tab-vault/">Tab Vault</a> ' +
			'is &copy; 2010&ndash;2011 <a href="http://chaosinacan.com">Joel Spadin</a>, ' +
			'images by <a href="http://dellustrations.deviantart.com/">Wendell Fernandes</a> ' +
			'and <a href="http://www.opera.com">Opera Software</a>',
	
}